...
const Navbar = () => {
  ...
  return (
    <header className="w-full max-w-7xl mx-auto relative inset-x-0 top-0 z-10">
      <Drawer showDrawer={isDrawerShown} toggleDrawer={setIsDrawerShown}/>
      <div className="px-4">
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          <button onClick={() => setIsDrawerShown(prev => !prev)} type="button" className="inline-flex p-2 ml-1 text-white transition-all duration-200 rounded-md sm:ml-4 lg:hidden focus:bg-gray-800 hover:bg-gray-800">
            {/* Menu open: "hidden", Menu closed: "block" */}
            <svg className={`${isDrawerShown ? "hidden": "block"} w-6 h-6`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16m-7 6h7" />
            </svg>

            {/* Menu open: "block", Menu closed: "hidden" */}
            <svg className={`${isDrawerShown ? "block" : "hidden"} h-6 w-6`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
